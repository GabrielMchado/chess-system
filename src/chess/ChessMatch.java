...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if (!type.equals("B") && !type.equals("N") && !type.equals("R") & !type.equals("Q")) {
			throw new InvalidParameterException("Invalid type for promoved"); 
		}
		
		PositionChessPiece pos = promoted.getChessPosition().toPosition();
	...
	private Piece makeMove(PositionChessPiece source, PositionChessPiece target) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == null) {
				PositionChessPiece pawnPosition;
				if (p.getColor() == Color.WHITE) {
					pawnPosition = new PositionChessPiece(target.getRow() + 1, target.getColumn());
				}
				else {
					pawnPosition = new PositionChessPiece(target.getRow() - 1, target.getColumn());
				}
	...
	private void undoMove(PositionChessPiece source, PositionChessPiece target, Piece capturedPiece) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				...
				PositionChessPiece pawnPosition;
				if (p.getColor() == Color.WHITE) {
					pawnPosition = new PositionChessPiece(3, target.getColumn());
				}
				else {
					pawnPosition = new PositionChessPiece(4, target.getColumn());
				}
